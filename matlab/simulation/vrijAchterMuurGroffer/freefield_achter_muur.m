...
%% grid parameters
D = 1.2;  %in m
L = 2.4;  %in m !!!x-y coordinate system

dx = 0.3e-3;
dz = dx;

%simt = 1e-2;%total simulation time in s
simt = 6.8e-3;%total simulation time in s
...
Dwidth = abs(Dright-Dleft);
Dheight = abs(Dbottom-Dtop);
...
%% record points
widthSteps = 11; %number of steps in one width of the wall
overshootSteps = 4; %steps to go before & beyond edge of wall
nrecx = widthSteps + 2*overshootSteps;
nrecz = overshootSteps;
stepx = Dwidth / (widthSteps - 1)
stepz = stepx;
...
xrecStart = Dleft - overshootSteps * stepx;
zrecStart = heigthD;
...
filename = ['freeFieldFloor_Texc',num2str(Texc),'_nexp',num2str(nexpx),'_exSize',num2str(exSize),'_dx',num2str(dx)]
save(['/tmp/',filename,'_all'], '-V7');
save(['/tmp/',filename,'_Precord'], 'Precord', '-V7');
